...
class GroqProvider(BaseLLM):
    ...
    def __init__(self):
        if not settings.GROQ_API_KEY:
            raise ValueError("GROQ_API_KEY is not configured in environment settings.")
    ...
    async def generate_response(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.error(f"Groq Authentication Error (No Retry): {e}")
                raise LLMServiceError("Groq authentication failed: Please verify your API credentials.")
            except (RateLimitError, APITimeoutError, APIError, OSError) as e:
                # Do not retry if it indicates exhausted quota / funds
                error_msg = str(e).lower()
                if "quota" in error_msg or "insufficient_funds" in error_msg or "balance" in error_msg:
                    logger.error(f"Groq Quota Exceeded (No Retry): {e}")
                    raise LLMServiceError("Groq API quota exceeded or insufficient funds.")
                
                last_exception = e
                logger.warning(
                    f"Groq transient error on attempt {attempt}/{max_retries}: {e}. "
                ...
                logger.error(f"Unexpected error in Groq Provider execution: {e}")
                raise LLMServiceError(f"Unexpected error: {str(e)}")
                
        # If we exhausted retries
        latency = time.perf_counter() - start_time
        logger.error(f"Groq calls failed after {max_retries} attempts: {last_exception}")
        
        # Specific classification for final exception
        if isinstance(last_exception, RateLimitError):
            raise LLMServiceError("Groq rate limit exceeded. Please try again later.")
        elif isinstance(last_exception, APITimeoutError):
            raise LLMServiceError("Groq request timed out.")
        else:
            raise LLMServiceError(f"Groq API call failed: {str(last_exception)}")
    ...
    async def _execute_request(self, input_messages: List[Dict[str, str]], config: Optional[GenerationConfig] = None) -> Any:
        """
        Executes the network call to the Groq API.
        """
        kwargs = {
    ...
    def _parse_response(self, response: Any, latency: float) -> LLMResult:
        """
        Parses the raw response object from client.chat.completions.create into LLMResult.
        """
